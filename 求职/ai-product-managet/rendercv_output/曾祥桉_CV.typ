...
- 文科背景转 AI 产品方向，具备 AI 产品从需求定义到 MVP 验证的实际交付经验。有 AI 产品助理实习与猎头行业经验，理解 B 端业务流程与用户需求。能独立完成原型设计、Demo 搭建与方案验证，擅长用结构化方法拆解模糊问题。英语可作为工作语言（雅思 6.5）。

== 核心能力

#strong[AI 产品设计:] 需求拆解｜场景定义｜PRD｜原型设计｜MVP 验证

#strong[AI 能力应用:] Prompt Engineering｜工具调用｜多模态集成｜AI Coding

#strong[B 端产品能力:] 流程设计｜异常兜底｜用户访谈｜竞品分析
...
#regular-entry(
  [
    ...
    #summary[项目背景 \     　• 为熟悉 AI 产品方法论与对话式交互设计，以出行下单场景为练习对象，完成完整 Concept Design。 \ 我的工作 \     　• 拆解下单全链路，设计语音对话流程（唤起 → 识别 → 补全 → 确认）。 \     　• 设计槽位置信度判断、追问补全与多轮对话逻辑。 \     　• 构建异常处理机制（超时、识别失败、兜底回退传统流程）。 \     　• 输出 PRD、流程图、异常兜底清单及交互原型。]
...
#regular-entry(
  [
    ...
    #summary[项目背景 \     　• 面向\"不愿精算食物\"的健身人群，基于拳掌法提供低门槛饮食记录与策略建议。 \ 我的工作 \     　• 以零代码背景独立完成端到端交付：产品定义、交互设计、开发部署、iOS 真机验证。 \     　• 设计低摩擦录入闭环：快速输入、预览卡片、下一步建议、历史复盘。 \     　• 集成多模态模型（图像识别 + 文本生成），处理多模型输出一致性与交互衔接。 \     　• 使用 AI Coding 工具完成 MVP 开发，Web 与 iOS 双端可运行。 \ 项目成果 \     　• 完成可演示闭环产品（真实设备运行），验证\"低成本记录路径\"可行性。]
...
#regular-entry(
  [
    ...
    #summary[项目背景 \     　• 针对多源信息获取低效问题，构建轻量化 AI 晨报系统。 \ 我的工作 \     　• 设计多源信息聚合 + 飞书推送流程。 \     　• 设计\"两级触发\"机制（普通版 \/ AI 版），平衡响应速度与资源消耗。 \     　• 在零成本约束下完成 MVP 闭环搭建。 \ 项目成果 \     　• 交付可运行方案，提升信息获取效率。]
